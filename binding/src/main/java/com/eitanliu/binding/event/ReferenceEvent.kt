...
import com.eitanliu.utils.weakReference
...
interface ReferenceConsumerAndResult<T, R> : UiEventConsumerResult<T, R>, ReferenceValue<T>,
    ReferenceResult<R>

inline fun <reified R> weakEventResult(
    event: UiEventResult<R>,
): ReferenceEventResult<R> = object : ReferenceEventResult<R> {

    private var weakResult: Reference<R> = weakReference()
...
): ReferenceConsumer<T> = object : ReferenceConsumer<T> {
    private var weak: Reference<T> = weakReference()
...
): ReferenceConsumerResult<T, R> = object : ReferenceConsumerResult<T, R> {
    private var weak: Reference<T> = weakReference()
...
inline fun <reified T, reified R> weakConsumerAndResult(
    consumer: UiEventConsumerResult<T, R>,
): ReferenceConsumerAndResult<T, R> = object : ReferenceConsumerAndResult<T, R> {
    private var weak: Reference<T> = weakReference()
